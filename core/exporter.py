...
class DataExporter:
    ...
    def export_entity(
        self, entity_name, start_date=None, end_date=None, date_field=None
    ):
        ...
        try:
            ...
            # Substituição do campo de data se fornecido
            if date_field:
                sql_query = sql_query.replace(":CAMPO_DATA", date_field)
            elif ":CAMPO_DATA" in sql_query:
                # Fallback se o SQL exige o campo mas não foi passado
                # Tenta detectar por contexto simples ou usa default
                if "contas_receber" in entity_name:
                    sql_query = sql_query.replace(":CAMPO_DATA", "d.RED_DATAVENCIMENTO")
                elif "contas_pagar" in entity_name:
                    sql_query = sql_query.replace(":CAMPO_DATA", "d.PAD_DATAVENCIMENTO")
