"""
Script para verificar e criar índices de otimização do Relatório de Faturamento
...
INDICES_NECESSARIOS = [
    {
        'nome': 'IDX_PEDIDO_CLIENTE',
        'tabela': 'PEDIDO',
        'coluna': 'CLI_CODIGO',
        'descricao': 'Índice por cliente em PEDIDO'
    },
    {
    ...
    {
        'nome': 'IDX_RECEBER_PEDIDO',
        'tabela': 'RECEBER',
        'coluna': 'PED_NUMEROOPERACAO',
        'descricao': 'Índice por pedido em RECEBER'
    },
    {
        'nome': 'IDX_RECEBTO_RECOP',
        'tabela': 'RECEBTO',
        'coluna': 'REC_NUMEROOPERACAO',
        'descricao': 'Índice por operação em RECEBTO'
    }
]
...
def verificar_indices_existentes(db):
    ...
    SELECT 
        ...
        RDB$UNIQUE_FLAG
    FROM RDB$INDICES
    WHERE RDB$RELATION_NAME IN ('PEDIDO', 'PEDITENS', 'RECEBER', 'RECEBTO')
...
def main():
    """Função principal"""
    print("=" * 60)
    print("OTIMIZAÇÃO DE PERFORMANCE DO RELATÓRIO DE FATURAMENTO")
    ...
    try:
        ...
        if criados > 0:
            ...
            print("  A performance do Relatório de Faturamento deve melhorar significativamente.")
            print("  Ganho esperado: de 6-11s para < 2s")
