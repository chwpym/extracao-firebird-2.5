    ...
    def _run_extraction_logic(self, start_dt, end_dt):
        try:
            ...
            for i, entity in enumerate(entities):
                pct = ((i + 1) / total) * 100
                self.window.after(0, lambda p=pct, e=entity: (self.progress_var.set(p), self.status_var.set(f"Exportando: {e}...")))
                
                self._log_to_gui(f"Processando: {entity}")
                success = exporter.export_entity(entity, start_dt, end_dt)
            ...
            self.window.after(0, lambda: (self.progress_var.set(100), self.status_var.set("Finalizado!")))
            db.close()
